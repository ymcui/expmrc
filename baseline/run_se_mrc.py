...
"""Run BERT on SQuAD and CMRC 2018."""
...
flags.DEFINE_string("task_name", "squad", "set task name, shoule be one of `squad`, `cmrc2018`.")
...
class SquadExample(object):
  ...
  def __repr__(self):
    ...
    if self.evidence_start_position:
      s += ", evidence_start_position: %d" % (self.evidence_start_position)
    if self.start_position:
      s += ", evidence_end_position: %d" % (self.evidence_end_position)
    return s
...
def read_squad_examples(input_file, is_training, repeat_limit=3):
  ...
  examples = []
  mis_match = 0
  evidence_mis_match = 0
  for article in train_data:
    for para in article['paragraphs']:
      context = para['context']
      if FLAGS.task_name == "squad":
        context_iter = context
      elif FLAGS.task_name == "cmrc2018":
  ...
  tf.logging.info("**********total: "+str(len(examples))+"**********")
  return examples
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      ...
      if example_index < 2:
        ...
        if is_training:
          ...
          tf.logging.info(
              "evidence: %s" % (tokenization.printable_text(evidence_text)))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, input_span_mask,
  ...
  (start_evidence_logits, end_evidence_logits) = (unstacked_evidence_logits[0], unstacked_evidence_logits[1])

  # apply output mask
  adder = (1.0 - tf.cast(input_span_mask, tf.float32)) * -10000.0
  start_evidence_logits += adder
  end_evidence_logits += adder

  return (start_logits, end_logits, start_evidence_logits, end_evidence_logits)
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (start_logits, end_logits, evidence_start_logits, evidence_end_logits) = create_model(
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      total_loss += float(FLAGS.loss_lambda) * (evidence_start_loss + evidence_end_loss) / 2
      ...
      predictions = {
          ...
          "evidence_start_logits": evidence_start_logits,
          "evidence_end_logits": evidence_end_logits,
      }
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
...
RawResult = collections.namedtuple("RawResult",
                                   ["unique_id", "start_logits", "end_logits", "evidence_start_logits", "evidence_end_logits"])
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    evidence_prelim_predictions = []
    for (feature_index, feature) in enumerate(features):  # multi-trunk
      result = unique_id_to_result[feature.unique_id]
      start_indexes = _get_best_indexes(result.evidence_start_logits, n_best_size)
      end_indexes = _get_best_indexes(result.evidence_end_logits, n_best_size)
      for start_index in start_indexes:
        for end_index in end_indexes:
          # We could hypothetically create invalid predictions, e.g., predict
          # that the start of the span is in the question. We throw out all
          ...
          evidence_prelim_predictions.append(
              _PrelimPrediction(
                  feature_index=feature_index,
                  start_index=start_index,
                  end_index=end_index,
                  start_logit=result.evidence_start_logits[start_index],
                  end_logit=result.evidence_end_logits[end_index]))

    evidence_prelim_predictions = sorted(
        evidence_prelim_predictions,
        key=lambda x: (x.start_logit + x.end_logit),
        reverse=True)
    ...
    seen_predictions = {}
    evidence_nbest = []
    for pred in evidence_prelim_predictions:
      if len(evidence_nbest) >= n_best_size:
        break
      feature = features[pred.feature_index]
      if pred.start_index > 0:  # this is a non-null prediction
        tok_tokens = feature.tokens[pred.start_index:(pred.end_index + 1)]
    ...
    best_non_null_evidence = ''
    for entry in evidence_nbest:
      total_scores.append(entry.start_logit + entry.end_logit)
      if best_non_null_evidence == '':
        if entry.text:
          best_non_null_evidence = entry.text
    ...
    all_predictions[example.qas_id] = {'answer': best_non_null_entry, 'evidence': best_non_null_evidence}
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    for result in estimator.predict(
      ...
      evidence_start_logits = [float(x) for x in result["evidence_start_logits"].flat]
      evidence_end_logits = [float(x) for x in result["evidence_end_logits"].flat]
      all_results.append(
          RawResult(
              unique_id=unique_id,
              start_logits=start_logits,
